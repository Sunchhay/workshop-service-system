...
export class CustomersService {
  ...
  async findAll(dto: QueryCustomerDto) {
    ...
    const { data, total } = await this.customersRepository.findAll({
      ...dto,
      page,
      limit,
    });
  ...
  async update(id: string, dto: UpdateCustomerDto) {
    ...
    if (dto.phone) {
      const existing = await this.customersRepository.findByPhone(
        dto.phone,
        id,
      );
  ...
  async updateStatus(id: string, dto: UpdateCustomerStatusDto) {
    ...
    const customer = await this.customersRepository.updateStatus(
      id,
      dto.status,
    );
  ...
  async remove(id: string) {
    ...
    const customer = await this.customersRepository.deactivate(id);

    return createResponse(customer, 'Customer deactivated');
  }
}
