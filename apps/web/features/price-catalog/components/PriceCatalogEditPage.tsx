...
export function PriceCatalogEditPage({ id }: PriceCatalogEditPageProps) {
  ...
  const [updatePriceCatalog, { isLoading: isUpdating }] = useUpdatePriceCatalogMutation();

  const handleSubmit = async (payload: CreatePriceCatalogRequest | UpdatePriceCatalogRequest) => {
    try {
      await updatePriceCatalog({ id, data: payload as UpdatePriceCatalogRequest }).unwrap();
          ...
          {isFetching ? (
            ...
            <PriceCatalogForm
              ...
              defaultValues={{
                ...
                customerType: entry.customerType ?? '__all',
