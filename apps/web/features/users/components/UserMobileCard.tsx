...
export function UserMobileCard({ user, onToggleStatus }: UserMobileCardProps) {
  ...
  return (
    <div
      ...
      tabIndex={0}
      onKeyDown={(e) => {
        if (e.key === 'Enter' || e.key === ' ') router.push(`/admin/users/${user.id}`);
      }}
    >
      {/* Avatar */}
      {user.imageUrl ? (
        <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-full">
          <Image src={user.imageUrl} alt={user.name} fill className="object-cover" sizes="40px" />
        </div>
      ) : (
      ...
      <div onClick={(e) => e.stopPropagation()}>
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            >
              {active ? t('users.confirmDisableTitle') : t('users.confirmEnableTitle')}
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
