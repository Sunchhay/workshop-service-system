...
interface AddProductItemDialogProps {
  open: boolean;
...
export function AddProductItemDialog({
  open,
  onOpenChange,
  onConfirm,
  isLoading,
}: AddProductItemDialogProps) {
  const { t } = useTranslation();

  const { data: productsData } = useGetProductsQuery({ status: 'ACTIVE', limit: 200 });
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: { productId: '', machineModelId: '__none', unitPrice: '', quantity: '1', note: '' },
  });
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { onOpenChange(o); if (!o) form.reset(); }}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{t('cart.addProduct')}</DialogTitle>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>
                    {t('cart.product')} <span className="text-destructive">*</span>
                  </FormLabel>
                  <Select onValueChange={field.onChange} value={field.value}>
                    <FormControl>
                      <SelectTrigger>
                        <SelectValue placeholder={t('cart.selectProduct')} />
                      </SelectTrigger>
                    </FormControl>
                    <SelectContent>
                      {products.map((p) => (
